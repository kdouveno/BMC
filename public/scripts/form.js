...
KDform = {
	objForm: function (sel) {
		...
		$(sel + " input[name]").each((i, o) => {
			if ($(o).hasClass("numberInput")) out[o.name] = parseInt(o.value);else if ($(o).attr("type") == "checkbox") out[o.name] = o.checked;else out[o.name] = o.value;
		});
		return out;
	},
	...
	numPlus: e => {
		if ($(e).is("fieldset[disabled] *")) return;
	...
	checkEvent: e => {
		var p = $(e).parent();
		if (e.checked) p.addClass("checked");else {
